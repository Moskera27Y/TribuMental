          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-white rounded-[40px] border border-[#ECE8E0] overflow-hidden shadow-sm">
              <div className="bg-[#8C9B73]/10 p-5 border-b border-[#ECE8E0] flex justify-between items-center">
                ...
                {hasMentalScore && (
                  <button onClick={() => setShowMentalTest(true)} className="text-[10px] font-bold text-[#8C9B73] hover:underline cursor-pointer">
                    Repetir Evaluación
                  </button>
                )}
              </div>
                ...
                {!hasMentalScore ? (
                  <div className="flex flex-col items-center text-center space-y-4 py-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-[#7A7875] max-w-xs leading-relaxed">Realiza nuestra evaluación profesional de 15 preguntas para que Tribu AI analice tu estado emocional actual.</p>
                    </div>
                    <button onClick={() => setShowMentalTest(true)} className="bg-[#2F3E46] text-white px-6 py-2.5 rounded-xl font-bold text-xs shadow-md hover:opacity-90 transition-all cursor-pointer">
                      ...
                      <div className="flex-1 space-y-4">
                        <div className="flex items-start gap-4">
                          <div className="shrink-0 flex flex-col items-center">
                            <div className="w-12 h-12 rounded-full border-4 border-[#8C9B73]/20 flex items-center justify-center relative">
                              ...
                              <svg className="absolute inset-0 w-full h-full -rotate-90">
                                <circle cx="24" cy="24" r="20" fill="transparent" stroke="#8C9B73" strokeWidth="4" strokeDasharray={`${(api.profile?.lastMentalHealthScore || 0) * 12.56} 125.6`} className="transition-all duration-1000" />
                              </svg>
                            </div>
                          ...
                          <div className="space-y-1.5 flex-1">
                            ...
                            <p className="text-[11px] text-[#7A7875] leading-relaxed italic border-l-2 border-[#8C9B73]/30 pl-3">"{api.profile?.mentalHealthProfile}"</p>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="w-full md:w-48 p-4 bg-[#FBF9F4] rounded-3xl border border-[#ECE8E0] space-y-2">
                          ...
                          <div className="h-16 flex items-end justify-between gap-1 px-1">
                            {api.profile.mentalHealthHistory.slice(-7).map((h, i) => (
                              <div key={i} className="w-full bg-[#8C9B73]/40 rounded-t-sm relative group" style={{ height: `${h.score * 10}%` }}>
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 bg-[#2F3E46] text-white text-[8px] py-0.5 px-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">{h.score}/10</div>
                              </div>
                            ))}
                          </div>
                        </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <p className="text-[10px] font-bold text-[#A3A19E] uppercase tracking-widest flex items-center gap-1.5">
                          <CheckCircle2 size={12} className="text-[#8C9B73]" /> Sugerencias de la IA para hoy
                        </p>
                        <button onClick={() => {
                          ...
                          <Share2 size={10} /> Compartir con mi Apoyo
                        </button>
                      </div>
  ...
  return (
    <>
      <Routes>
        ...
        <Route path="/onboarding" element={api.user ? <OnboardingFlow userName={api.user.name} onComplete={handleCompleteOnboarding} /> : <Navigate to="/login" />} />
        <Route path="/dashboard/*" element={
          api.user && api.profile?.onboarded ? (
            <ThemeLayout
              ...
              user={api.user} profile={api.profile} subscription={api.subscription}
              viewOwnerId={api.viewOwnerId} setViewOwnerId={api.setViewOwnerId}
              companionRelations={api.companionRelations} receivedInvitations={api.receivedInvitations}
              onLogout={() => api.logout()} onOpenCrisis={() => setShowCrisisAlert(true)}
            >
              {showCrisisAlert ? <HelpCrisisCentre contacts={api.supportContacts} /> : (
                <>
                      ...
                      <div className="bg-white rounded-[40px] border border-[#ECE8E0] p-6 md:p-8 shadow-sm space-y-6">
                        <div className="flex items-center justify-between border-b border-[#ECE8E0] pb-4">
                          <h4 className="text-lg font-serif font-semibold text-[#2F3E46] flex items-center gap-2">
                            <User className="w-5 h-5 text-[#8C9B73]" /> Perfil Materno
                          </h4>
                          <span className="px-3 py-1 rounded-full bg-[#8C9B73]/10 text-[#5A634D] font-bold text-[10px] uppercase tracking-wider">Plan {api.subscription?.plan || 'Gratis'}</span>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-4">
                            ...
                            <div>
                              ...
                              <input type="text" value={api.user?.name} onChange={(e) => api.login(e.target.value, api.user?.email || '')} className="w-full px-4 py-3 rounded-2xl border border-[#ECE8E0] focus:outline-none bg-[#FBF9F4]" />
                            </div>
                            <div>
                              <label className="block text-xs font-semibold text-gray-500 mb-1.5">Ciudad</label>
                              <input type="text" value={api.profile.city || ''} onChange={(e) => api.updateProfile({ city: e.target.value })} className="w-full px-4 py-3 rounded-2xl border border-[#ECE8E0] focus:outline-none bg-[#FBF9F4]" />
                            </div>
                            <div>
                              <label className="block text-xs font-semibold text-gray-500 mb-1.5">Pareja</label>
                              <input type="text" value={api.profile.partnerName || ''} onChange={(e) => api.updateProfile({ partnerName: e.target.value })} className="w-full px-4 py-3 rounded-2xl border border-[#ECE8E0] focus:outline-none bg-[#FBF9F4]" />
                            </div>
                          </div>
                          <div className="space-y-4">
                            <p className="text-[10px] font-bold text-[#A3A19E] uppercase tracking-widest">Etapa Actual</p>
                            <select value={api.profile.status} onChange={e => api.updateProfile({ status: e.target.value as any })} className="w-full px-4 py-3 rounded-2xl border border-[#ECE8E0] focus:outline-none bg-[#FBF9F4]">
                              <option value={PregnancyStatus.PREGNANT}>Embarazada 🤰</option>
                              <option value={PregnancyStatus.POSTPARTUM}>Posparto 👶</option>
                            </select>
                            <div className="grid grid-cols-2 gap-3">
                              <input type="number" value={api.profile.weeksOrMonths} onChange={e => api.updateProfile({ weeksOrMonths: parseInt(e.target.value) || 0 })} className="w-full px-4 py-3 rounded-2xl border border-[#ECE8E0] focus:outline-none bg-[#FBF9F4]" />
                              <input type="date" value={api.profile.status === PregnancyStatus.PREGNANT ? api.profile.expectedDueDate : api.profile.babyBirthDate} onChange={e => api.updateProfile(api.profile?.status === PregnancyStatus.PREGNANT ? { expectedDueDate: e.target.value } : { babyBirthDate: e.target.value })} className="w-full px-4 py-3 rounded-2xl border border-[#ECE8E0] focus:outline-none bg-[#FBF9F4] text-[10px]" />
                            </div>
                          </div>
                        </div>
                        <div className="pt-6 border-t border-[#ECE8E0] flex justify-end">
                          <button onClick={() => api.logout()} className="px-6 py-3 rounded-2xl border border-rose-100 text-rose-500 font-bold hover:bg-rose-50 flex items-center gap-2 cursor-pointer"><Lock size={16} /> Cerrar Sesión</button>
                        </div>
                      </div>
                    </div>
                  )}
      ...
      {showPaymentSuccess && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[36px] max-w-md w-full p-8 text-center space-y-6 shadow-2xl relative">
            <Sparkles className="w-8 h-8 text-orange-400 mx-auto" />
            <h3 className="text-2xl font-serif text-[#2F3E46] font-bold">¡Bienvenida! 🌸</h3>
            <button onClick={() => setShowPaymentSuccess(false)} className="w-full bg-[#8C9B73] text-white py-3 rounded-xl font-bold text-xs cursor-pointer">Comenzar 🌸</button>
          </div>
        </div>
      )}
    </>
